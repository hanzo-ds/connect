...
from datastore_connect.datatypes.base import DatastoreType, type_map, ch_read_formats, ch_write_formats
...
def format_map(fmt_map: Optional[Dict[str, str]]) -> Dict[Type[DatastoreType], str]:
    if not fmt_map:
        return {}
    final_map = {}
...
def _matching_types(pattern: str, fmt: str = None) -> Sequence[Type[DatastoreType]]:
    ...
    if not matches:
        raise ProgrammingError(f'Unrecognized Datastore type {pattern} when setting formats')
    if fmt:
        invalid = [ch_type.__name__ for ch_type in matches if fmt not in ch_type.valid_formats]
        if invalid:
            raise ProgrammingError(f"{fmt} is not a valid format for Datastore types {','.join(invalid)}.")
    return matches
