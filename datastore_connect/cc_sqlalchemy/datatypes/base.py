...
from datastore_connect.datatypes.base import DatastoreType, TypeDef, EMPTY_TYPE_DEF
from datastore_connect.datatypes.registry import parse_name, type_map
from datastore_connect.driver.binding import str_query_value
...
class ChSqlaType:
    """
    A SQLAlchemy TypeEngine that wraps a DatastoreType.  We don't extend TypeEngine directly, instead all concrete
    subclasses will inherit from TypeEngine.
    """
    ch_type: DatastoreType = None
    generic_type: None
    _ch_type_cls = None
    _instance = None
    ...
    def __init_subclass__(cls):
        """
        Registers ChSqla type in the type map and sets the underlying DatastoreType class to use to initialize
        ...
        if not cls._ch_type_cls:
            try:
                ...
                logger.warning('Attempted to register SQLAlchemy type without corresponding Datastore Type')
                return
    ...
    def __init__(self, type_def: TypeDef = EMPTY_TYPE_DEF):
        """
        Basic constructor that does nothing but set the wrapped DatastoreType.  It is overridden in some cases
        ...
        :param type_def: TypeDef tuple used to build the underlying DatastoreType.  This is normally populated by the
        parse_name function
        """
    ...
    @name.setter
    def name(self, name):  # Keep SQLAlchemy from overriding our Datastore name
        pass
    ...
    def _compiler_dispatch(self, _visitor, **_):
        ...
        when generating the type name.  The underlying DatastoreType generates the correct name for the type
...
def sqla_type_from_name(name: str) -> ChSqlaType:
    """
    Factory function to convert a Datastore type name to the appropriate ChSqlaType
    :param name: Name returned from Datastore using Native protocol or WithNames format
    :return: ChSqlaType
    """
