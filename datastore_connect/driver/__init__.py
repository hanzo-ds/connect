...
import datastore_connect.driver.ctypes
from datastore_connect.driver.client import Client
from datastore_connect.driver.common import dict_copy
...
from datastore_connect.driver.asyncclient import AsyncClient
...
def create_client(*,
                  ...
                  **kwargs) -> Client:
    """
    The preferred method to get a Datastore Connect Client instance

    :param host: The hostname or IP address of the Datastore server. If not set, localhost will be used.
    :param username: The Datastore username. If not set, the default Datastore user will be used.
      Should not be set if `access_token` is used.
    :param password: The password for username.
      Should not be set if `access_token` is used.
    :param access_token: JWT access token (Datastore Cloud feature).
      Should not be set if `username`/`password` are used.
    :param database:  The default database for the connection. If not set, Datastore Connect will use the
    ...
    :param port: The Datastore HTTP or HTTPS port. If not set will default to 8123, or to 8443 if secure=True
      or interface=https.
    ...
    :param settings: Datastore server settings to be used with the session/every request
    :param generic_args: Used internally to parse DBAPI connection strings into keyword arguments and Datastore settings.
    ...
    :param compress: Enable compression for Datastore HTTP inserts and query results.  True will select the preferred
    ...
    :param client_name: client_name prepended to the HTTP User Agent header. Set this to track client queries
      in the Datastore system.query_log.
    ...
    :param ca_cert: If verify is True, the file path to Certificate Authority root to validate Datastore server
     certificate, in .pem format.  Ignored if verify is False.  This is not necessary if the Datastore server
    ...
    :param session_id Datastore session id.  If not specified and the common setting 'autogenerate_session_id'
    ...
    :param server_host_name  This is the server host name that will be checked against a TLS certificate for
      validity.  This option can be used if using an ssh_tunnel or other indirect means to an Datastore server
      where the `host` argument refers to the tunnel or proxy and not the actual Datastore server
    :param autogenerate_session_id  If set, this will override the 'autogenerate_session_id' common setting.
    :return: Datastore Connect Client instance
    """
    if dsn:
        parsed = urlparse(dsn)
...
def default_port(interface: str, secure: bool):
    ...
    raise ValueError('Unrecognized Datastore interface')
...
async def create_async_client(*,
                              ...
                              **kwargs) -> AsyncClient:
    """
    The preferred method to get an async Datastore Connect Client instance.
