...
from datastore_connect.datatypes.container import Array, Tuple, Map, Nested
from datastore_connect.datatypes.network import IPv4, IPv6
from datastore_connect.datatypes.numeric import BigInt, Float32, Float64, Enum, Bool, Boolean, Decimal
from datastore_connect.datatypes.registry import get_from_name
from datastore_connect.datatypes.special import UUID
from datastore_connect.datatypes.string import String, FixedString
from datastore_connect.datatypes.temporal import Date, Date32, DateTime, DateTime64
from datastore_connect.driver.common import array_sizes
...
def random_col_data(ch_type: Union[str, DatastoreType], cnt: int, col_def: RandomValueDef = RandomValueDef()):
    """
    Generate a column of random data for insert tests
    :param ch_type: DatastoreType or Datastore type name
    :param cnt: Number of values to generate
    :param col_def: Parameters to use for random data generation
    :return: A tuple of length cnt of random Python data values of the requested DatastoreType
    """
...
def random_value_gen(ch_type: DatastoreType, col_def: RandomValueDef):
    """
    Returns a generator function of random values of the requested DatastoreType
    :param ch_type: DatastoreType to generate
    ...
    raise ValueError(f'Invalid Datastore type {ch_type.name} for random column data')
...
def random_tuple(element_types: Sequence[DatastoreType], col_def):
...
def random_nested(keys: Sequence[str], types: Sequence[DatastoreType], col_def: RandomValueDef):
...
gen_map: Dict[Type[DatastoreType], Callable] = {
