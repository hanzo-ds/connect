...
def main():
    host = os.getenv('DATASTORE_CONNECT_TEST_HOST', 'localhost')
    port = int(os.getenv('DATASTORE_CONNECT_TEST_PORT', '8123'))
    password = os.getenv('DATASTORE_CONNECT_TEST_PASSWORD', '')
    ...
    for database_row in database_result:
        ...
        if database.startswith('dbt_datastore') or database.startswith('datastore_connect'):
