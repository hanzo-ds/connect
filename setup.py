...
try:
    ...
    c_modules = cythonize('datastore_connect/driverc/*.pyx', language_level='3str')
...
def run_setup(try_c: bool = True):
    ...
    if os.path.isfile('.dev_version'):
        ...
        with open(os.path.join(project_dir, 'datastore_connect', '__version__.py'), encoding='utf-8') as version_file:
            file_version = version_file.read().strip()
            match = re.search(r"version\s*=\s*'(.+)'", file_version)
            if match is None:
                raise ValueError(f'invalid version {file_version} in datastore_connect/__version__.py')
